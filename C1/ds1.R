...
x
(v2 = c(1,2,3)) #round bracket . create and print the vector
v2
class(v2)
...
(v4 = 100:200)
(v4a = 1:5000) # stored entirely but printed only upto 1000 numbers
length(v4) # count of items in vector
v4[5:10] # pint sequence numbers from a vector
v4[c(4,10,25,35)] # print random numbers from a vector by using c()
v5 = seq(1,10,2)
v4; v5  #two commands seperated by semicolon
v4 # the [1],[28] in the execution part shows the position of the item in the vector
v4[-c(1,3,4)]  #exclued postion 1,3,4 by using -c()
...
(taxila=100:105)
names(taxila)
names(taxila)=c('sampada', 'vinesh', 'samvin', 'vinsam' ) # alloting names to numbers
taxila
...
matrix1[seq(1,4,2),]
rowSums(matrix1[seq(1,4,2),]) # sum of every alternative row
...
floor(12.67)  #remove decimals and shows lower value 12
ceiling(12.67) #remove decimals and shows higher value 13
